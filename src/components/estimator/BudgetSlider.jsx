...
export function BudgetSlider({ value, onChange, budgetValue, setBudgetValue, formattedBudgetText, budgetRange }) {
  ...
  return (
    <div className="space-y-3 bg-zinc-950/60 p-3.5 sm:p-4 rounded-xl border border-zinc-800/80">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <label className="text-[11px] sm:text-xs font-mono font-bold text-zinc-300 uppercase tracking-wider">
          2. Alokasi Budget Proyek:
        </label>
        <span
          data-testid="budget-value-display"
          className="self-start sm:self-auto text-xs sm:text-sm font-mono font-bold text-cyan-400 bg-cyan-500/10 px-2.5 py-1 rounded border border-cyan-500/20"
        >
          {displayVal}
        </span>
      </div>
      ...
      <div className="relative pt-1">
        ...
        />
        <div className="flex justify-between text-[9px] sm:text-[10px] font-mono text-zinc-400 pt-2">
